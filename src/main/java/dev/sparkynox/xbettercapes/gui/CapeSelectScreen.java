...
/**
 * Cape selection GUI.
 * Grid of cape cards (image + name), click to equip.
 * URL input field at bottom for custom image-host capes.
 */
public class CapeSelectScreen extends Screen {
    ...
    protected void init() {
        ...
        }).dimensions(centerX + 86, bottomY - 24, 60, 18).build());
    ...
    private void drawCapeCard(DrawContext ctx, CapeEntry entry, int x, int y,
        ...
        // Card bg
        int bg = selected ? 0xFF0D2A36 : (hovered ? 0xFF181828 : 0xFF0F0F1C);
        ctx.fill(x, y, x + CARD_W, y + CARD_H, bg);

        // Border (glow effect for selected)
        int border = selected ? 0xFF00E5FF : (hovered ? 0xFF334455 : 0xFF1E1E30);
        ctx.drawBorder(x, y, CARD_W, CARD_H, border);
        if (selected) {
        ...
        if (tex != null && entry.resourcePath != null) {
            // 1.21.4 drawTexture requires RenderLayer — use gui textured
            ctx.drawTexture(
        ...
        int nameColor = selected ? 0x00E5FF : (hovered ? 0xEEEEEE : 0xAAAAAA);
        ...
        ctx.drawCenteredTextWithShadow(textRenderer, Text.literal(name),
                x + CARD_W / 2, y + CARD_H - 11, nameColor);
