...
public class SkinScreen extends Screen {
    ...
    protected void init() {
        ...
        this.addDrawableChild(ButtonWidget.builder(
                Text.literal("Mode: Player Name"), btn -> {
                    ...
                    btn.setMessage(Text.literal("Mode: " +
                            (mode.equals("player") ? "Player Name" : "Texture URL")));
                    updatePlaceholder();
        ...
        this.addDrawableChild(ButtonWidget.builder(Text.literal("Apply Skin"),
                btn -> applySkin())
    ...
    private void applySkin() {
        ...
        if (mode.equals("player")) {
            ...
            if (!input.startsWith("http://") && !input.startsWith("https://")) {
                statusMsg = "URL must start with http(s)://"; return;
            }
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        String current = SkinConfig.selectedSkin.equals("NONE")
                ? "No skin set" : SkinConfig.selectedSkin;
        if (current.length() > 38) current = current.substring(0, 35) + "...";
        ctx.drawCenteredTextWithShadow(textRenderer,
                Text.literal("Current: " + current), cx, cy - 22, 0x446677);

        String status = !SkinFetcher.statusMsg.isEmpty() ? SkinFetcher.statusMsg : statusMsg;
        if (!status.isEmpty()) {
